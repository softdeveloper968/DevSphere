...
namespace MyDMVpro.Common
{
    ...
    public static class ClaimsHelpers
    {
        ...
        {
            var userid = principal.Claims.Where(c => c.Type == MyDmvProClaims.UserId).FirstOrDefault();
            if (userid == null)
                return "";
            return userid.Value;
        }
        public static bool IsGroupAdmin(ClaimsPrincipal principal)
        {
            var isGroupAdmin = principal.Claims.Where(c => c.Type == MyDmvProClaims.IsGroupAdmin).FirstOrDefault();
            if (isGroupAdmin == null)
                return false;
            return isGroupAdmin.Value == "true";
        }
    }
}
