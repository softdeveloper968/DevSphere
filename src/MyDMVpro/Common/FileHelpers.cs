...
namespace MyDMVpro.Common
{
    ...
    {
        public static int MaxUnitSize = 100000;
        public static int SizeUnitMultiplier = 1024;
        public static string SizeUnits = "KB";
        public static int MaxFileUploadSize = MaxUnitSize * SizeUnitMultiplier;
            ...
            ModelStateDictionary modelState)
        {
            ...
            // a display name.
            MemberInfo property =
                typeof(FileUploads).GetProperty(
            ...
            else if (formFile.Length > MaxFileUploadSize)
            {
                modelState.AddModelError(formFile.Name,
                    $"The {fieldDisplayName}file ({fileName}) exceeds {MaxFileUploadSize / SizeUnitMultiplier} {SizeUnits}.");
            }
            else
            {
                try
        ...
        public static byte[] ProcessBinaryFormFile(IFormFile formFile,
            ModelStateDictionary modelState)
        {
            var fieldDisplayName = string.Empty;
            ...
            else
            {
                try
                {
                    ...
                    {
                        fileContents = reader.ReadBytes((int)formFile.Length);
